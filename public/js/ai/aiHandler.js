...
const AIHandler = {
  ...
  toggleAIMode() {
    ...
    // 如果关闭AI模式，清除输入框中的AI标识
    if (!this.isAIMode) {
      this.clearAIInputIndicator();
    }

    // 显示模式切换提示
    const message = this.isAIMode
      ? CONFIG.SUCCESS.AI_MODE_ENABLED
      : CONFIG.SUCCESS.AI_MODE_DISABLED;
    ...
    try {
      ...
      if (window.API && typeof API.sendAIMessage === "function") {
        const result = await API.sendAIMessage(
          content,
          "ai-system",
          CONFIG.MESSAGE_TYPES.AI_RESPONSE
        );
      ...
      console.error("AIHandler: 存储AI回复时出错", error);
      // 降级处理：直接在前端显示
      this.addMessageDirectly({
  ...
  updateResponse(chunk, fullResponse) {
    if (
      ...
      window.UI &&
      typeof UI.updateAIResponse === "function"
    ) {
  ...
  async handleAIError(error) {
    ...
    if (
      ...
      window.UI &&
      typeof UI.removeMessage === "function"
    ) {

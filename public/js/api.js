...
const API = {
  ...
  async request(url, options = {}) {
    ...
    const authHeaders = Auth
      ? Auth.addAuthHeader(defaultOptions.headers)
  ...
  async uploadFile(file, deviceId, onProgress = null) {
    try {
      ...
      if (!Utils.validateFileSize(file.size)) {
        throw new Error(
          typeof CONFIG.ERRORS.FILE_TOO_LARGE === "function"
            ? CONFIG.ERRORS.FILE_TOO_LARGE()
            : CONFIG.ERRORS.FILE_TOO_LARGE
        );
      }
      ...
      if (onProgress) {
        return this.uploadWithProgress(
          CONFIG.API.ENDPOINTS.FILES_UPLOAD,
          formData,
          onProgress
        );
      }
  ...
  async deleteMessage(messageId) {
    try {
      const response = await this.delete(
        `${CONFIG.API.ENDPOINTS.MESSAGES}/${messageId}`
      );
